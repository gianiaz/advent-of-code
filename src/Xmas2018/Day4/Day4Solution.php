...
class Day4Solution implements SolutionInterface, SecondPartSolutionInterface
{
    ...
    {
        $this->timestamps = iterator_to_array($this->createSortedTimestamps($input ?? $this->defaultInput()));
        ksort($this->timestamps);
    }
    ...
    private function createSortedTimestamps(string $input): \Generator
    {
        ...
        foreach ($rows as $row) {
            $timestamp = TimestampFactory::create($row);
            yield $timestamp->getTimeString() => $timestamp;
        }
    }

    public function getGuardIdThatSleptMostMinutes(): int
    {
        $sleepMinutes = [];
        $fallenAsleep = null;
        $currentGuard = null;
        ...
        foreach ($this->timestamps as $timestamp) {
            if ($timestamp instanceof BeginShift) {
                $currentGuard = $timestamp->getGuardId();
            } elseif ($timestamp instanceof FallsAsleep) {
                $fallenAsleep = $timestamp;
            } elseif ($timestamp instanceof WakesUp) {
                if (! $fallenAsleep instanceof FallsAsleep) {
                    throw new \RuntimeException('Wakes up without falling asleep before!');
                }

                if (! $currentGuard) {
                    throw new \RuntimeException('No current guard!');
                }

                if (! array_key_exists($currentGuard, $sleepMinutes)) {
                    $sleepMinutes[$currentGuard] = 0;
                }

                $sleepMinutes[$currentGuard] += $timestamp->getMinutes() - $fallenAsleep->getMinutes();
            } else {
                throw new \InvalidArgumentException('Unrecognized timestamp: ' . \get_class($timestamp));
            }
        }

        return (int) \array_search(\max($sleepMinutes), $sleepMinutes, true);
    }
    ...
    private function getSleepPatternByGuard(): array
    {
        ...
        foreach ($this->timestamps as $timestamp) {
            ...
            if ($timestamp instanceof FallsAsleep) {
                $fallenAsleep = $timestamp;

                continue;
            }

            if ($timestamp instanceof WakesUp) {
                if (! $fallenAsleep instanceof FallsAsleep) {
                    throw new \RuntimeException('Wakes up without falling asleep before!');
                }
    ...
    {
        return '[1518-11-22 00:49] wakes up
