...
class Day1Solution implements SolutionInterface, SecondPartSolutionInterface
{
    ...
    public function solveSecondPart(array $modules = self::INPUT)
    {
        $total = 0;

        foreach ($modules as $module) {
            $total += $this->calculateWithAdditionalFuel($module);
        }

        return $total;
    }

    public function calculateWithAdditionalFuel(int $moduleWeight)
    {
        $amountOfFuel = $this->calculateFuel($moduleWeight);
