...
namespace Jean85\AdventOfCode\Xmas2019\Day3;
...
class Day3Solution implements SolutionInterface, SecondPartSolutionInterface
{
    ...
    {
        return $this->getClosestCrossing(self::INSTRUCTIONS1, self::INSTRUCTIONS2);
    }

    public function solveSecondPart()
    {
        return $this->getShortestCrossing(self::INSTRUCTIONS1, self::INSTRUCTIONS2);
    }

    /**
     * @param string[] $instructions1
     * @param string[] $instructions2
     */
    public function getClosestCrossing(array $instructions1, array $instructions2): int
    {
        $this->drawWire($instructions1);

        return $this->findClosestCrossing($instructions2);
    }
     ...
     * @param string[] $instructions2
     */
    public function getShortestCrossing(array $instructions1, array $instructions2): int
    {
        $this->drawWire($instructions1);

        return $this->findShortestCrossing($instructions2);
    }

    /**
     * @param string[] $instructions
     */
    private function drawWire(array $instructions): void
    {
        $x = 0;
        $y = 0;
        $steps = 0;
        ...
        foreach ($instructions as $instruction) {
            $this->applyInstruction($instruction, $x, $y, function ($x, $y) use (&$steps) {
                $this->grid[$x][$y] = ++$steps;
            });
        }
    }

    private function applyInstruction(string $instruction, int &$x, int &$y, callable $callable): void
    {
        ...
        while ($distance--) {
            switch ($direction) {
                ...
                case 'L':
                    ...
                    break;
                default:
                    throw new \InvalidArgumentException('Unrecognized direciton: ' . $direction);
            }
    ...
    private function findClosestCrossing(array $instructions): int
    {
        $closest = PHP_INT_MAX;
        $x = 0;
        $y = 0;
        ...
        foreach ($instructions as $instruction) {
            $this->applyInstruction($instruction, $x, $y, function ($x, $y) use (&$closest) {
    ...
    private function findShortestCrossing(array $instructions): int
    {
        ...
        foreach ($instructions as $instruction) {
            $this->applyInstruction($instruction, $x, $y, function ($x, $y) use (&$shortest, &$steps) {
                ++$steps;
                if (isset($this->grid[$x][$y])) {
                    $shortest = min($shortest, $this->grid[$x][$y] + $steps);
                }
            });
        }
